...
- **Here is the specific development process for Python plugins: **
- First, execute `pip install --user pdm` to install pdm package management tool
- Next, create a new directory for plugin development. Start the command line in this directory and execute `pdm init` command. Follow the prompts of the pdm tool to create a new project and fill in the information about the project.
...
- Next, create the `__init__.py` file and write the plugin code in it. When loading the plugin, the Python interpreter will read and execute this file.
- If necessary, you can create multiple source files and use import to include them. Properly splitting the source code into files helps improve the clarity of the project structure and facilitates further maintenance and extension of the plugin.
