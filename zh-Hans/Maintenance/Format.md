...
LiteLoader
├─include            # 头文件
│  ├─liteloader      # LiteLoader 私有头文件
│  └─llapi           # LiteLoader API 头文件
│      ├─db          # 数据库 API 头文件
│      ├─event       # 事件 API 头文件
│      ├─impl        # 实现 API 头文件
│      ├─mc          # Minecraft API 头文件
│      ├─perm        # 权限 API 头文件
│      └─utils       # 工具 API 头文件
├─src                # 源文件
│  ├─liteloader      # LiteLoader 私有源文件
│  └─llapi           # LiteLoader API 源文件
│      ├─command     # 命令 API 源文件
│      ├─db          # 数据库 API 源文件
│      ├─mc          # Minecraft API 源文件
│      ├─nbt         # NBT API 源文件
│      ├─network     # 网络 API 源文件
│      ├─scoreboard  # 计分板 API 源文件
│      └─utils       # 工具 API 源文件
└─third-party        # 第三方库
    ├─include        # 第三方库头文件
    ├─lib            # 静态库
...
*注意：若您为了实现一个功能使用了多个文件，请建立文件夹包含所有这些文件。文件夹的命名规范同命名空间，如`perm`、`db`*
...
&emsp;&emsp;读者可能很快指出这两个循环可能有不同的语义：如果容器正在发生增删，那么 `BB->end()` 可能会在每次循环中改变它的值，而第二个循环实际上可能不会是正确的。如果您确实依赖此行为，请以第一种形式编写循环并添加注释，表明您是故意这样做的。
